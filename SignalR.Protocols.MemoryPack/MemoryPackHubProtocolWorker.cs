...
{
    internal class MemoryPackHubProtocolWorker
    {
        ...
        public MemoryPackHubProtocolWorker(MemoryPackSerializerOptions memoryPackSerializerOptions)
        {
        ...
        private static bool TryParseBinaryMessage(ref ReadOnlySequence<byte> buffer, out ReadOnlySequence<byte> payload)
        {
            if (buffer.IsEmpty)
            {
                payload = default;
        ...
        private static HubMessage? ParseHubMessage(ref MemoryPackReader reader, IInvocationBinder binder)
        {
            if (!reader.TryReadCollectionHeader(out int itemCount))
            {
                return null;
            }
        ...
        private static int LengthPrefixLength(long length)
        {
            var lenNumBytes = 0;
            do
            {
        ...
        private static void WriteLengthPrefix(long length, IBufferWriter<byte> output)
        {
        ...
        private static int WriteLengthPrefix(long length, Span<byte> output)
        {
        ...
        private void WriteMessageCore<TWriter>(HubMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            switch (message)
            {
        ...
        private static void WriteInvocationMessage<TWriter>(InvocationMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
        ...
        private static void WriteStreamInvocationMessage<TWriter>(StreamInvocationMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            writer.WriteCollectionHeader(6);
        ...
        private static void WriteStreamingItemMessage<TWriter>(StreamItemMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
        ...
        private static void WriteCompletionMessage<TWriter>(CompletionMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            var resultKind =
        ...
        private static void WriteCancelInvocationMessage<TWriter>(CancelInvocationMessage message, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
        ...
        private static void WritePingMessage<TWriter>(ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            writer.WriteCollectionHeader(1);
            writer.WriteValue<byte>(HubProtocolConstants.PingMessageType);
        }

        private static void PackHeaders<TWriter>(IDictionary<string, string>? headers, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            if (headers != null)
            {
        ...
        private static HubMessage CreateInvocationMessage(
        ...
        private static HubMessage CreateStreamInvocationMessage(
            ref MemoryPackReader reader,
            IInvocationBinder binder,
            int itemCount)
        {
        ...
        private static HubMessage CreateStreamItemMessage(ref MemoryPackReader reader, IInvocationBinder binder)
        {
        ...
        private static CompletionMessage CreateCompletionMessage(ref MemoryPackReader reader, IInvocationBinder binder)
        {
            var headers = ReadHeaders(ref reader);
            var invocationId = ReadString(ref reader, "invocationId");
        ...
        private static CancelInvocationMessage CreateCancelInvocationMessage(ref MemoryPackReader reader)
        {
            var headers = ReadHeaders(ref reader);
            var invocationId = ReadString(ref reader, "invocationId");
        ...
        private static CloseMessage CreateCloseMessage(ref MemoryPackReader reader, int itemCount)
        {
        ...
        private static object?[] BindArguments(ref MemoryPackReader reader, IReadOnlyList<Type> parameterTypes)
        {
            ...
            try
            {
                ...
                {
                    // messagepack 中会交给抽象方法DeserializeObject交由子类去实现，这里直接 memorypack 的实现
        ...
        private static Dictionary<string, string>? ReadHeaders(ref MemoryPackReader reader)
        {
        ...
        private static string[]? ReadStreamIds(ref MemoryPackReader reader)
        {
        ...
        private static string? ReadString(ref MemoryPackReader reader, string filedName)
        {
            try
            {
        ...
        private static T? ReadValue<T>(ref MemoryPackReader reader, string filedName)
        {
            try
            {
        ...
        private static object? ReadValue(ref MemoryPackReader reader, Type type, string filedName)
        {
            try
            {
        ...
        private static void WriteStreamIds<TWriter>(string[]? streamIds, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            if (streamIds != null)
            {
        ...
        private static void WriteArgument<TWriter>(object? argument, ref MemoryPackWriter<TWriter> writer) where TWriter : IBufferWriter<byte>
        {
            if (argument == null)
            {
